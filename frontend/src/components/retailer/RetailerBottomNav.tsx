...
import { Home, Smartphone, History, Percent, User } from 'lucide-react';
...
export const RetailerBottomNav: React.FC<RetailerBottomNavProps> = ({
  ...
  return (
    <nav className="sm:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl border-t border-slate-200 dark:border-slate-800 px-2 py-1.5 flex items-center justify-around shadow-lg dark:shadow-2xl safe-area-bottom">
      {/* Tab 1: Home */}
      <button
        type="button"
      ...
      >
        <div className={`p-1 rounded-xl transition-colors ${currentTab === 'RECHARGE' ? 'bg-blue-500/15 dark:bg-blue-500/20' : ''}`}>
          <Smartphone className="w-5 h-5" />
        </div>
        <span className="text-[10px] tracking-tight">Recharge</span>
      </button>

      {/* Tab 3: Passbook / Transactions */}
      <button
        type="button"
      ...
      <button
        ...
        className="flex flex-col items-center gap-1 py-1 px-3 rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all active:scale-95"
      >
